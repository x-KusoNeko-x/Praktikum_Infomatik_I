...
{
    for (int j = 0; j < GROESSE_Y; j++)// Index beginnt mit 0
    {
...
int gewinner(const int spielfeld[GROESSE_Y][GROESSE_X])
{
    ...
    if (zaehlerSpieler1 < zaehlerSpieler2)
    {
        return 2;//player2 gewinnt
    }
    else
    {
        return 1;//player1 gewinnt
    }
}
            ...
            while (aufSpielfeld(x, y))
            {
                ...
                else if (spielfeld[y][x] == aktuellerSpieler)
                {
                    if (gegnerGefunden) 
                    {
                        ...
                        while (kx != x || ky != y)
                        {
                            spielfeld[ky][kx] = aktuellerSpieler;
                            kx += i;
                            ky += j;
                        }
                        break;
...
void spielen(const int spielerTyp[2])
{
    ...
    bool spielerKannZiehen[2] = { true, true };

    while (spielerKannZiehen[0] || spielerKannZiehen[1])// nur wenn die Beide falsch sind, stoppt das Spiel
    {
        if (spielerTyp[aktuellerSpieler - 1] == MENSCH)
        {
            spielerKannZiehen[aktuellerSpieler - 1] = menschlicherZug(spielfeld, aktuellerSpieler);
        }
        else
        {
            // Hier würde der Computerzug implementiert werden, aber jetzt noch nicht fertig
            spielerKannZiehen[aktuellerSpieler - 1] = computerZug(spielfeld, aktuellerSpieler); // Beispielweise auf false gesetzt
        }

        zeigeSpielfeld(spielfeld);
        aktuellerSpieler = 3 - aktuellerSpieler;// Wechseln zwischen den 2 Players
...
int main()
{
    ...
    char spieler1;
    char spieler2;
    std::cout << "ist Spieler 1 Computer?(j/n)";
    std::cin >> spieler1;
